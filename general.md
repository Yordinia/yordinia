...
- Chat GPT Project Ideas - Say you've now learn, and you want to do a project. - If you want to find a project to do about Naruto
...
- creating symbolic link - mklink pip.exe pip3.11.exe > not on windows
  on windows
...
- curl -v localhost: 8080 - header symbol request VS response headers > VS <
  to download the code of the first page we're receiving
- `wget localhost:7600`

### POSTING using curl

- curl -X POST -H "Content-Type: application/json" -d '{"name":"SampleFlow"}' http://localhost:8080/api/flows
...
- curl -X PUT -H "Content-Type: application/json" -d '{"name":"UpdatedFlow"}' http://localhost:8080/api/flows/1

### DELETing

- curl -X DELETE http://localhost:8080/api/flows/1

Or you can use javascript fetch

```
// GET
fetch('http://localhost:8080/api/flows')
...
fetch('http://localhost:8080/api/flows', {
    ...
    body: JSON.stringify({ name: 'NewFlow' }),
})
    .then(response => response.json())
    .then(data => console.log(data))
    .catch(error => console.error('Error:', error));
...
```
  @GetMapping("/{id}")
    public ResponseEntity<Flow> getFlowById(@PathVariable Long id) {
        ...
        return optionalFlow.map(flow -> new ResponseEntity<>(flow, HttpStatus.OK))
                           .orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

```

## POST with Create a new flow
...
```
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Flow> updateFlow(@PathVariable Long id, @RequestBody Flow updatedFlow) {
        Optional<Flow> optionalExistingFlow = flowRepository.findById(id);
        return optionalExistingFlow.map(existingFlow -> {
            existingFlow.setName(updatedFlow.getName());
            existingFlow.setValidityPeriod(updatedFlow.getValidityPeriod());
            // Update other attributes as needed

            Flow savedFlow = flowRepository.save(existingFlow);
...
- do class X extends JpaRepository<Step(TYpe), Long(Id)>
- x.findAll() . .to find all xes
...
// Usage
Box stringBox = new Box();  // Raw type usage
stringBox.setValue("Hello");
String greeting = (String) stringBox.getValue();  // Need explicit casting
```

## Good Pricatices

- It's a good practice to have consistency in the return types between the service and the controller methods.
...
- To return a void response entity

```
 public ResponseEntity<Void> empty (boolean x) {
    if(x)
